...
describe Answers, 'when creating' do
  ...
  it 'should find any answer title elements' do
    title_elements = Answers.find_title_elements H("<html>#{@written_answers_title}#{@date}#{@title}</html>")
    title_elements.size.should == 1
    title_elements.first.inner_text.to_s.should == @title_text
  end

  it 'should find date' do
    Answers.find_date(H(@date)).should == Date.parse(@date_text)
  end

  it 'should find title text' do
    html = H(@title)
    Answers.find_title_text(html).should == @title_text
  end

  it 'should find major title text' do
    Answers.find_major_title_text(@title_text).should == @major_title
    Answers.find_major_title_text('Autism').should == 'Autism'
    Answers.find_major_title_text('British Overseas Territories').should == 'British Overseas Territories'
  end

  it 'should find minor title text' do
    Answers.find_minor_title_text(@title_text).should == @minor_title
    Answers.find_minor_title_text('Autism').should == nil
    Answers.find_minor_title_text('British Overseas Territories').should == nil
  end
