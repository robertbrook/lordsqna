...
class Answers
  ...
  class << self
    def create_answers title_element
    ...
    def find_answering_member element
      name = find_answering_name element
      name[/\((.+)\)/, 1] || name
    end

    def find_answering_role element
      name = find_answering_name element
      name[/(.*)\s+\(.+\)/, 1]
    end

    def is_an_answer_start? element
      contains_named_anchor element, /^wa_st_\d+$/
    end

    def find_answer_initial_paragraph element
      paragraph = nil
    ...
    def is_a_non_initial_answer_paragraph? element
      contains_named_anchor element, /^wa_stpa_\d+$/
    end

    def find_answer_paragraphs answer_initial_paragraph
    ...
    def find_answer_paragraphs_text answer_initial_paragraph
    ...
    private
      ...
      def contains_named_anchor element, pattern
        (a = element.at('a')) && (name = a['name']) && !name.to_s[pattern].nil?
      end

      def find_answering_name element
