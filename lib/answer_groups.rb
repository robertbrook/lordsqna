...
class AnswerGroups
  ...
  class << self
    ...
    def find_title_elements doc
      (doc/'h3').delete_if {|h| h.inner_text.to_s[/written answers/i]}
    end

    def find_title_text element
      element.inner_text.to_s.strip
    end

    def find_title_anchor_name title_element
      (a = title_element.at('a')) && (name = a['name']) ? name : nil
    end

    def find_major_title_text text
      text.split(': ').first
    end

    def find_minor_title_text text
